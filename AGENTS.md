...
Ralph Wiggum is an autonomous AI coding loop CLI tool that runs Codex iteratively to work through a Product Requirements Document (PRD). It reads requirements from `prd.json`, implements features one at a time, verifies work through tests, updates progress in `progress.txt`, and commits changes automatically.
...
2. **PRD Planning** (`--plan` flag) - Invokes Codex in plan mode to generate a structured PRD from natural language requirements

3. **Iteration Loop** (main logic) - Executes Codex with `--dangerously-skip-permissions` in a loop, each iteration:
...
4. **Completion Detection** - Monitors Codex's output for `RALPH_COMPLETE` (all done) or `RALPH_NEEDS_HELP` (stuck) signals
