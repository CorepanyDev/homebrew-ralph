...
- Claude model and effort configuration via `RALPH_CLAUDE_MODEL` and `RALPH_CLAUDE_EFFORT`.
- Terminal output showing the selected Claude model and effort before Ralph starts Claude.

### Changed

- Ralph now defaults Claude Code runs to the `opus` model alias with `max` effort.

## [1.4.0] - 2026-02-05

### Added

- `--max-tasks N` option for `--plan` command to configure maximum task count
  - Default remains 30 if not specified
  - Usage: `ralph --plan --max-tasks 50` or `ralph --plan --max-tasks 50 "description"`
...
[1.4.1]: https://github.com/CorepanyDev/homebrew-ralph/releases/tag/v1.4.1
[1.4.0]: https://github.com/CorepanyDev/homebrew-ralph/releases/tag/v1.4.0
